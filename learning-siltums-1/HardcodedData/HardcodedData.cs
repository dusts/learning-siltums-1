...
namespace learning_siltums_1.HardcodedData
{
    public class HardcodedData
    {
        ...
        static string s2_a1 = $"Kurināmie ir vielas, kas sadegot izdala siltumu, dabā iegūstamas lielos daudzumos un degšanas produkti nav ļoti kaitīgi. Tos iedala:\n -> Pēc sastāva: organiskie (satur oglekli, ūdeņradi), metāliskie (attiecas uz raķešu tehniku), kodolkurināmie\n -> Pēc izcelsmes: fosīlie (augu un dzīvnieku paliekas) un biokurināmie (biomasa - salmi, koksne, etc)\n -> Pēc ieguves: dabiskie (iegūst dabā un tādus kā ir izmanto dedzināšanā - ogles, etc), mākslīgie (tiešā veidā dabā nav atrodami - benzīni, dīzeļdegvielas, etc)\n -> Pēc agregātstāvokļa: cieti, šķidri un gāzveida\nKurināmo sastāvs:\n -> Cietie un šķidrie kurināmie:\n --> Darba sastāvs - C{needsSuperscript}d{needsSuperscript} + H{needsSuperscript}d{needsSuperscript} + O{needsSuperscript}d{needsSuperscript} + S{needsSuperscript}d{needsSuperscript} + N{needsSuperscript}d{needsSuperscript} + A{needsSuperscript}d{needsSuperscript} + W{needsSuperscript}d{needsSuperscript} = 100% masas %. A(ash) - pelni (nedegošie savienojumi), W(wet) - mitrums.\n --> Sausais sastāvs - C{needsSuperscript}s{needsSuperscript} + H{needsSuperscript}s{needsSuperscript} + O{needsSuperscript}s{needsSuperscript} + S{needsSuperscript}s{needsSuperscript} + N{needsSuperscript}s{needsSuperscript} + A{needsSuperscript}s{needsSuperscript} = 100% (darba sastāvam noņem nost mitrumu)\n -> Degošais sastāvs - C{needsSuperscript}dg{needsSuperscript} + H{needsSuperscript}dg{needsSuperscript} + O{needsSuperscript}dg{needsSuperscript} + S{needsSuperscript}dg{needsSuperscript}{needsSubscript}g{needsSubscript} + N{needsSuperscript}dg{needsSuperscript} = 100% (noņemam nost nedegošo jeb pelnus). S{needsSuperscript}dg{needsSuperscript} ietver gan degošos gan nedegošos savienojumus, tāpēc {needsSubscript}g{needsSubscript} pie S norāda uz degošo daļu - organiskie un metāliskais savienojums FeS{needsSubscript}2{needsSubscript}(Pieits)\n -> Gāzveida kurināmie:\n --> {captialSigmaChar}C{needsSubscript}n{needsSubscript}H{needsSubscript}m{needsSubscript} + N{needsSubscript}2{needsSubscript} + CO{needsSubscript}2{needsSubscript} + CO + H{needsSubscript}2{needsSubscript}S + H{needsSubscript}2{needsSubscript}Otv.+...= 100% (tilpuma %)\nC{needsSubscript}n{needsSubscript}H{needsSubscript}m{needsSubscript} ietilpst: metāns - CH{needsSubscript}4{needsSubscript}, etnāns - C{needsSubscript}2{needsSubscript}H{needsSubscript}6{needsSubscript}, propāns - C{needsSubscript}3{needsSubscript}H{needsSubscript}8{needsSubscript}, butāns - C{needsSubscript}4{needsSubscript}H{needsSubscript}10{needsSubscript}, pentāns - C{needsSubscript}5{needsSubscript}H{needsSubscript}12{needsSubscript}.\n";
        static string s2_q2 = $"Kurināmo degšanas reakcijas. Gaisa pārpilnības koeficients.";
        static string s2_a2 = $"Degšana ir degošā elementa ķīmiskā reakcija ar skābekli, kuras rezultātā izdalās siltums. Elementi - ūdeņradis, sērs un ogleklis. Ūdeņradis un sērs deg tieši.\nŪdeņraža (H{subscript2}) tiešā degšana:\n H{subscript2} + 0.5O{subscript2} = H{subscript2}O + kcal/kmol (ūdens šķidrā(68212)/tvaika veidā(57768))\nSēra (S) tiešā degšana:\n S + O{subscript2} = SO{subscript2} + 70860 kcal/kmol\nTiešā C (oglekļa) degšana:\n C + O{subscript2} = CO{subscript2} + 97000 kcal/kmol \nPakāpeniska C (oglekļa) degšana:\n C + 0.5O{subscript2} = CO + 29409 kcal/kmol (nepilnīga sadegšana, trūkst skābeklis)\n CO + 0.5O{subscript2} = CO{subscript2} + 67591 kcal/kmol\n     Kopā: 97000 kcal/kmol\nGaisa pārpalikuma jeb pārpilnības koeficients {smallAlphaChar} (alfa) (air excess factor) parāda, cik reizes vairāk gaisa nekā teorētiski nepieciešams jāpievada kurtuvei, lai būtu pilnīga sadegšana.\n(no prezentācijas)";
        static string s2_q3 = $"Kurināmā augstākā un zemākā sildspēja. To noteikšana. Nosacītais kurināmais.";
        static string s2_a3 = $"Kurināmā augstākā sildspēja Q{needsSubscript}A{needsSubscript} (siltumspēja, sadegšanas siltums) (angl. gross calorific value) ir siltuma daudzums, ko iegūst pilnībā sadedzinot vienu vienību (1kg vai 1m{supscript3}) kurināmā (kJ/kg vai kcal/kg vai kWh/kg - gāzveida uz kubikmetru).\nKurināmā zemāko sildspēju (siltspēju) Q{needsSubscript}z{needsSubscript} (net calorific value) iegūst no augstākās sildspējas atņemot siltuma daudzumu, kas tiek patērēts iztvaicēšanai.\n Q{needsSubscript}z{needsSubscript} = Q{needsSubscript}A{needsSubscript} - Q{needsSubscript}W{needsSubscript}\n Q{needsSubscript}W{needsSubscript} = 25*(9H{needsSuperscript}d{needsSuperscript} + W{needsSuperscript}d{needsSuperscript}).\nNosacītais kurināmais (conditional fuel) jeb akmeņogļu ekvivalients kalpo, lai varētu salīdzināt dažādus kurināmos.\n -> Ogļu ekvivalents (CE - coal equivalent): tas ir pieņemts kurināmais, kura zemākā siltumspēja Q{needsSubscript}Z{needsSubscript} ir pielīdzināta labām akmeņoglēm un ir vienāda ar 29300 kJ/kg (jeb 7000 kcal/kg)\n -> Naftas ekvivalents (toe - tonnes of oil equivalent): tas ir pieņemts lielums, lai salīdzinātu enerģijas daudzumu, izmantojot dažādus kurināmos ar dažādām sildspējām un arī elektroenerģiju, 1toe ir ekvivalents enerģijas daudzumam 41898GJ jeb 11630 MWh.\n(no prezentācijas)";
        static string s2_q4 = $"Akmeņogles: to veidi, izcelsme, sastāvs, sildspēja.";
        static string s2_a4 = $"-Veidi:\n -- Brūnogles (lignīts, degakmens, degslānis). Satur daudz ūdens un pelnu, samērā maz oglekļa, zema sildspēja\n -- Akmeņogles veidojas no brūnoglēm ap 3km dziļumā\n -- Antracīts veicojas līdz 6km dziļumā, satur līdz 96% oglekļa, dod sevišķi daudz siltuma, taču grūti uzliesmo.\nAntracītam līdzīgas īpašības ir koksam: iegūšt no akmeņoglēm atdalot gaistošos savienojumus.\nAkmeņogles veidojas apstākļos, kuros augu atliekas uzrkājas ātrāk nekā notiek to sadalīšanās baktēriju iedarbībā. Parasti šādi apstākļi ir purvos. Sākotnējā ogļu veidošanās stadija ir kūdra. Tai nokļūstot zem citiem nogulumiežiem spiediena iedarbībā no kūdras izdalās ūdens un gāzveida savienojumi. Zēmāka spiediena ietekmē veidojas brūnogles, augstāka spiediena un temperatūras ietekmē - akmeņogles un antracīts.\nAkmeņogļu sastāvs:\n - vidējais gaistošo vielu saturs sasniedz 35-40%\n - pelnu saturs 2-45%\n - mitruma saturs 4-15%\n - sildspēja no 2500 (brūnoglēm) līdz 7200 (antracītam) kcal/kg\n - S 0.3-7%\n - C 75-97%\n - H{subscript2} 1.5 - 5.7%\n - O{subscript2} 1.5 - 15%\n - N{subscript2} līdz 1.5%\n- Iedalījums pēc gaistošo savienjumu satura (gaistošo vielu sastāvā ir pirmdarva (brūnogles), akmeņogļu darva (akmeņoglēs) un gāzes: oglekļa oksīds, ūdeņradis, metāns, viegli gaistošie ogļudeņraži un citi):\n - Brūnogles 45-55%\n - Garliesmas 45-55%\n - Gāzes 30-45%\n - Treknās 15-30%\n - Liesās 4-15%\n - Antracīts 0-4%\n - Kokss 0-4%\n(no prezentācijas)";
        static string s2_q5 = $"Kūdra: tās izcelsme, ieguve, sastāvs, sildspēja. Kūdras briketes.";
        static string s2_a5 = $"Kūdra veidojas apstākļos, kuros augu atliekas uzkrājas ātrāk nekā notiek to sadalīšanās baktēriju iedarbībā. Parasti šādi apstākļi ir purvos. Iegūst virs zemes ar traktortehniku.\nIr gabalkūdra un frēzkūdra. Frēzkūdra ir sausāka bet ir nepieciešams vairāk tehnikas lai apstrādātu un vairāk braucienus lai savāktu.\nSastāvs:\n -Slapjai kūdrai\n -- pelnu saturs 0.1-2%\n -- mitruma saturs 85-96%\n -Gaissaisai kūdrai\n -- pelnu saturs 0.4-9%\n -- mitruma saturs 20-30%\n -- sēra saturs līdz 0.5%.\nAptuvenais degošais sastāvs:\n -- Ogleklis C - 50%\n -- Ūdeņradis H{subscript2} - 6%\n -- Skābeklis O{subscript2} - 33%\n -- Slāpeklis N{subscript2} - 1.5%\n -- Sērs S - 0.5%\nSildspēja gaissausai kūdrai 2300-3500 kcal/kg\nBlīvums 330-400 kg/m{supscript3}\nKūdras briketes iegūst no frēzkūdras sapresējot to 1.2 MPa spiedienā un žāvējot.\nĪpašības:\n -- Q{needsSubscript}Z{needsSubscript}=4300-5300 kcal/kg\n -- Pelnu saturs 6-15%\n -- Mitrums 10-17%\n -- Sērs aptuveni 0.2-0.3%\n -- Īpatnējais svars 600-700 kg/m{supscript3}";
        static string s2_q6 = $"Koksnes kurināmie: to sastāvs, sildspēja, mitrums.";
        static string s2_a6 = $"Malka, kokskaidu granulas, šķelda, kokskaidu briketes.\n-Malka: malka pieder pie biokurināmajiem. Koksnes izmešu bilanci ir pieņemts uzskatīt par nulli. Ir emisijas faktori. Koksni nocērtot, tai ir ļoti liels mitrums. Apmēram 50-60%. Gaissausai (vienkārši žāvēta) malkai mitrums var būt 20-25%. Siltumspēja zaļai malkai 1600kcal/kg, gaissausai 3300-3500kcal/kg (~14000kJ/kg). Ja speciāli žāvē līdz 15% mitruma, tad var sasniegt 15000kJ/kg.\nSvaigi cirstai malkai W=40-80% un Q{needsSubscript}Z{needsSubscript}=1600-1800 kcal/kg.\nŽāvētai malkai W{needsSuperscript}d{needsSuperscript}=20-25%, A{needsSuperscript}d{needsSuperscript}=0.1-0.5% zaļai koksnei, A{needsSuperscript}d{needsSuperscript}=0.2-0.8% gaissausai malkai un Q{needsSubscript}Z{needsSubscript}=3000-3500 kcal/kg.\nMalkas degošais sastāvs no C-50%, H{subscript2}-6%, O{subscript2}-44%, N{subscript2} un S-0%.\nSausas malkas svars no 350-540kg/m3.\n-Kokskaidu briketes: Koka skaidas sapresē pie paaugstinātas temperatūras veidojot apaļas vai kantainas briketes. Paaugstinātā temperatūrā notiek žūšana un izdalās viela, kas satur/salīmē skaidas. Mitrums W līdz ar to ir 8-12%, siltumspēja briketēm ir ~4200-4400kcal/kg jeb 17500-18500kJ/kg. Pelnu saturs A=1-2%. Briketes ir jākombinē ar citu kurināmo, jo nepilnīgas degšanas rezultātā sablīvējās un veidojas CO, kas izraisa . Ir 4 veidu briketes - apaļas (izjūk degot, sliktāka degšana), četrkantainas (ilgāk saglabā formu degot, neizjūk. deg lēnāk un ilgāk nekā apaļās.), mizu/nakts briketes, speciālās/kamīna briketes (brūna virsma ar cauru vidu - dūmgāzes nesaplēš briketes dēļ cauruma un ir termiski apstrādāta, kas noņem vairumu darvas).\n-Kokskaidu granulas - sapresētas koksnes skaidas, presējot ir veikta koksnes žāvēšana. Mēdz būt 6 un 8mm diametrā. Gaumes lieta. Pēc garumā līdz 3cm. Netiek liktas papildus vielas klāt lai salīmētos, jo temperatūras rezultātā izdalās vielas kas salipina visu kopā. Siltumspēja: 4400kcal/kg, mitrums W=10-12%, tilpumsvars 550kg/m3, pelnu saturs A=1-2%. Ar granulām var automatizēt mazās katlumājas/mājas apkuri.\n-Šķelda: tā ir koksne vai koksnes pārpalikumi kuri tiek sašķeldoti. Pēc izmēriem - smalkāka līdz 3cm, rupjāka 5-6cm. Var būt divu veidu šķelda atkarībā no izcelsmes: 1) nāk no koksnes pārstrādes ražotnes, šī šķelda ir prognozējama ar prognozējamiem parametriem, parasti tā ir ar zemu mitrumu; 2) meža šķelda - dažādu cirsmu pārpalikumi, sākot ar zariem - beidzot ar mizām, šķeldas īpašības nav prognozējamas, ir augstu mitrumu (W). Šo šķeldu piegādā siltumapgādes uzņēmumam. 2 varianti - tiek ilgstoši uzturēta un samazinās mitrums vai arī tiek īsā laikā patērēta ar lielu mitrumu. Gaisā žuvusi šķelda mitrums W=20-25%, mākslīgi žuvusi ar mitrumu W=20%. Ātrāk izžāvējot šķelda mazāk bojājas un ir augstvērtīgāka. Norēķins ar piegādātāju tiek darīts 2 veidos: ja prognozējamas, tad par beramkubu; ja nav prognozējamas, tad par iegūto enerģiju no šķeldas. Ja šķeldai ir klāt putekļi, tad tās var radīt eksplozijas, tāpēc pie katliem ir smidzinātāji kas iesmidzina sausā šķeldā ūdeni.\nKoksnes kurinājamos mitrumu nosaka nosverot paraugu un pēc tam 24h šo paraugu žāvējot 105grādu C temperatūrā, pēc tam atkal nosver un starpība ir izdalītais mitrums.\n";
        static string s2_q7 = $"Naftas pārtvaice, tajā iegūtie produkti, to izmantošana.";
        static string s2_a7 = $"Naftas pārtvaice (fizikālais process) -naftu iztvaicē un atdala frakcijas ar dažādām iztvaikošanas temperatūrām. Ir 2 kolonnas aparātā - atmosfēras un vakuuma kolonna. Uz atmosfēras kolonnu padod jēlnaftu un iegūst -> gāzi (pati vieglākā, izmanto ķīmiskajā rūpniecībā), benzīns (iztvaikošnas t 50-200C), ligroīns (t 110-230C, izmanto reaktīvo dzinēju degvielā), petroleja (t 140-300C, izmanto reaktīvo dzinēju degvielā), gazoils (t 230-330C), solāreļļa (t 280-380C). Ja nesadala sīkāk petrolejā, gazoilā un solāreļļā, tad iegūst dīzeļdegvielu šajā diapozonā. Paliek pāri mazuts (60-80% no visa).\n Mazuts tālāk tiek novadīts uz vakuumiekārta, kas veido vakuumu un neizmantoto mazutu aizvada uz vakuuma kolonnu, kur iegūst 4 frakcijas un pārpalikumu -> aparātu eļļas (vieglākā, mazāk viskoza, dažādu mehānismu eļļošanai), vidējās industriālās eļļas (izmanto hidrosistēmās - traktoriem, ekskavātoriem, utml.), motoreļļas (dabiskās motoreļļas), smagās industriālās eļļas (lieto reduktoros utml.) un pāri paliek gudrons, kas ir piķim līdzīga viela, un to var, piemēram, izmantot ceļu būvē \n";
        static int s2_z7 = Resource.Mipmap.shema_naftas_partveice;
        static string s2_q8 = $"Benzīni, to īpašības, markas. Sastāvs. Oktānskaitlis, tā noteikšana.";
        static string s2_a8 = $"Benzīnu izmanto iekšdedzes motoros, ģeneratoros.\nĪpašības: Qz~10500kcal/kg (42000-44000kJ/kg), blīvums:~700-770kg/m3, viskozitāte: pie 0C - 0,65cst. Antidetonācijas īpašības - saspiežot līdz noteiktam spiedienam sākas degvielas pašaizdegšanās, process ir eksplozīvveidīgs, ar ļoti lielu ātrumu un to sauc par detonāciju. Tāpēc ir markas - A95, A98 - šie cipari raksturo antidetonācijas īpašības. Šīs īpašības degvielai var noteikt 2 veidos: 1) ar motormetodi - izmanto stenda motoru ar maināmu kompresijas pakāpi, darbinot ar doto benzīnu, palielina kompresiju līdz sākas detonācija; 2) pētnieciskā metode: degvielas sastāvu ķīmiski analizē. Abos veidos degvielu pielīdzina etalonam kas sastāv no izooktāna C8H18 (grūti detonē) un normālhektāns C7H16 (ātri detonē), līdz ar to ciparu markā sauc par oktānskaitli: iekš A95 - 95 ir oktānskaitlis un A95=95%C8H18 un 5%C7H16.\n Lai paaugstinātu oktānskaitli lieto piedevas. Kādreiz lietoja tetraetilsvinu, to sauc par etilēto benzīnu - ļoti kaitīgs jo izplūdes gāzes satur svinu. ES izmanto MTBE - metabutilesteris. + ir obligāti jāliek klāt metilspirtu/etilspirtu, kas palīdz pacelt oktānskaitli (samazinās degvielas siltumspēja, bet uzlabo degšanas procesu).\n -Frakcijas:\n -- Vieglās frakcijas - uzlabo iedarbināšanu pie zemām temperatūrām.\n -- Darba frakcijas\n -- Smagās frakcijas\n -- Latvijā marku nosaukumā norāda pētniecisko oktānskaitli. Jo augstāks oktānskaitlis, jo augstāka izturība pret detonāciju.\n -Oktānskaitlis: oktānskaitlis atbilst izooktāna daudzumam etalonmaisījumā, procentos\n -- Rezultāti atšķiras - nosakot ar metormetodi oktānskaitlis ir nedaudz mazāks, nekā ar pētniecisko.\n -- Pētnieciskais oktānskaitlis raksturo benzīna veiktspēju zemas slodzes apstākļos, savukārt motora oktānskaitlis – augstas slodzes apstākļos.\n -Sastāvs:\n -- svins (max 5mg/l),\n -- sērs (max 10 mg/kg),\n -- mangāns (max 2 mg/l),\n -- sveķi (max 5 mg/100ml),\n -- ogļūdeņraži (olefīni max 18% tilpuma, aromātiskie ogļūdeņraži max 35% tilpuma),\n -- benzanols (max 1% tilpuma),\n -- skābeklis (max 2.7% masas),\n -- metanols (max 3% tilpuma),\n -- etanols (max 5% tilpuma),\n -- izopropilspirts, izobutilspirts, tercobutilspirts, ēteri (5 un vairāk C atomi) un citi skābekļa savienojumi (max 2.7% tilpuma)\n";
        static string s2_q9 = $"Dīzeļdegviela, tās īpašības, markas. Sastāvs. Cetānskaitlis.";
        static string s2_a9 = $"Dīzeļdegvielu raksturo cetānskaitlis ir noteikts ar standartu - robežas ir 45-50. Dīzeļdegvielas sadegšana noris 4 etapos: aizdegšanās aizkavēšanās periods, ātras degšanas periods, lēnas degšanas periods, atlikuma sadegšana. Jo mazāks cetānskaitlis, jo ilgāks aizkavēšanās periods, tātad mazāks lietderīgums. Pie lieliem cetānskaitļiem notiek ļoti ātra degšana, skābeklis nepaspēj visur iesaistīties un zūd lietderīgums.\n -Īpašības:\n -- Blīvums=820-860 kg/m{supscript3}.\n -- Viskozitāte=1.5-8 cst.(sabiezēšanas un sastingšanas temperatūras - A0,A1,A2, pie sabiezēšanas var viedoties parafīni, kas nogulsnējās un traucē cirkulāciju; pie sastingšanas temperatūras degvielai pazūd tecēšanas īpašības)\n -- Q{needsSubscript}Z{needsSubscript}=42500-44000 kJ/kg.\n\n-Sastāvs: Cetānskaitlis - (min)51, (max) blīvums 15C temp - 845kg/m3, (max) destilācija: temperatūra, kad iztvaicēti 95% degvielas - 360C, (max) policikliskie aromātiskie ogļūdeņraži - 8%, (max) sēra saturs - 10 mg/kg, (max) taukskābiju metilestera saturs (FAME) - 7%\n\nCentānskaitli nosaka uz stenda, salīdzinot ar etalonu, kurš sastāv no cetāna C{needsSubscript}16{needsSubscript}H{needsSubscript}34{needsSubscript} un {smallAlphaChar}-metil-naftalīna C{needsSubscript}10{needsSubscript}H{needsSubscript}7{needsSubscript}CH{needsSubscript}3{needsSubscript}.\nDīzeļdegvielai ir limitēts sēra saturs.\n";
        static string s2_q10 = $"Mazuts un šķidrais krāšņu kurināmais (destilāts), sastāvs, īpašības, ieguve, izmantošana.";
        static string s2_a10 = $"Mazutu dala pēc viskozitātes un sadala markās - M20 (visšķidrākais), M40, M60, M80, M100, M200 - šis skaitlis raksturo viskozitāti. Viskozitātes un sacietēšanas dēļ mazutu silda, lai varētu sūknēt pa cauruļvadu.\nMazuts ir naftas pārstrādes procesa starpprodukts.\nQ{needsSubscript}Z{needsSubscript}=40000-42000 kJ/kg.\nPastāv arī flotes jeb kuģu mazuts. To markas ir F5 līdz F12 ar viskozitāti no 5-12 cst.\nDēļ lielās viskozitātes mazuts ir jāsilda, lai varētu padot uz kurināmo katlu (t=80-100).\nMazutam ir diezgan augsts sēra (S) saturs -> S=0.15-3.5%, kā arī liels ūdens daudzums (W<=2%), kas pirms padeves uz degli ir jāatdala, nostādinot. A=0.2-1%.\nKrāšņu kurināmo (destilāts) iegūst sajaucot naftas pārstrādes frakcijas ar dažādām iztvaikošanas temperatūrām. Var iekļaut vieglākas (ātrāk aizdegās) un smagākas (lielāka siltumatdeve). Tas ir augstvērtīgs kurināmais ar augstu sildspēju Q{needsSubscript}Z{needsSubscript}=40000-42000kJ/kg=10150 kcal/kg. Tam ir zema viskozitāte, to nevajag uzsildīt (derīgas nelielām katla iekārtām) un zems sēra saturs -> S līdz 0.005%.\n";
        static string s2_q11 = $"Metilspirts, tā iegūšana, īpašības, izmantošana.";
        static string s2_a11 = $"Metilspirts jeb koka spirts. Metilspirtu iegūst lapu koku destruktīvā destilācijā jeb termiski sadalot koksni. Koksnes sadalīšana notiek slēgtā iekārtā pakāpeniski paagustinoties temperatūrai no 160 līdz 450 grādiem C. Process ir eksotermisks. Papildus siltums jāpievada tikai lai sāktos reakcija. Tālākā procesā siltums tiek izdalīts. Rezultātā koksne sadalās un veidojas sekojoši produkti: degoša gāze (singāze), šķidrums jeb kondensāts, koka ogle. Reakcijā rodas degoša gāze (var izmantot apsildei), šķidrums (satur 1-6% CH{needsSubscript}3{needsSubscript}OH, 9-12% CH{needsSubscript}3{needsSubscript}COOH (etiķskābe) un 0.1-0.5% CH{needsSubscript}3{needsSubscript}COOCH{needsSubscript}3{needsSubscript} (acetons), bet pārējais ir H{subscript2}0. Pāri paliek cietā frakcija jeb koka ogle. Tālāk veicot šķidruma seperāciju un destilāciju iegūst no 92 līdz 95% metilspirta. Lai iegūtu augstāku spirta saturu, to ķīmiski apstrādā ar CaCl{subscript2}.\nNo 1 stera koksnes iegūst ~8l metilspirta.\nMetilspirtu var iegūt arī sintētiski: iegūst no CO un 2H{subscript2} katalizātora klātbūtnē (ZnO vai Cr2O3 pie 300-400C 200-300bar) iegūst metilspirtu CH3OH.\nGalvenās īpašības: siltumspēja - Qz=~23000kJ/kg=5490kcal/kg, ķīmiskais sastāvs - C-37.5%, H2-12.5%, O2-50%, iztvaikošanas temperatūra ts=64.5C, blīvums=792kg/m3.\nPielietojums: lieto kā piedevu benzīnam līdz 10% kā biopiedevu, kura arī paaugstina oktānskaitli.\n";
        static string s2_q12 = $"Etilspirts, tā iegūšana, īpašības, izmantošana.";
        static string s2_a12 = $"Etilspirts C2H5OH. Iegūšana:\n1) no graudiem vai sakņaugiem: galvenā sastāvdaļa ciete/stērķele - C6H10O5, izejmateriālu samaļ un iemaisa ūdenī, pievieno enzīmu un termiski apstrādā (vāra 30-180minūtes), šajā procesā ciete pārveidojas maltozē (disaharīds), tālāk produktu atdzesē līdz ~60-62C un piejauc sašķelšanās enzīmu un maltoze pārvēršas par glikozi;\n2) Cukurbietes vai cukurniedres, kā arī augļi: izejmateriāls ir sukroze (C12H22O11), to samaļ un iemaisa ūdenī, pievieno enzīmu un procesa rezultātā sukroze pārvēršās glikozē(C6H12O6) un fruktozē (C6H12O6);\n3) augu stublāji, lapu koksne vai papīrs: izejmateriāls ir celuloze, samaļ un iemaisa ūdenī, pievieno sālskābi vai sērskābi, un paaugstinātā spiedienā vāra pie temp. 180-220C, vāra līdz celuloze pārvēršas glikozē.\n Iegūtajam produktam (1,2,3) pievieno raugu un sākas fermentācija (C6H12O6+H2O->raugs->2C2H5OH+2CO2+brāga), beigās sanāk etilspirts, CO2 un brāga. Gala iznākumam stiprums ir 10%. Tālāk tiek veikta destilācija un var iegūt līdz 95.6% etilspirtu.\nJa vajadzīgs augstāks spirta saturs tad pievieno kalcija oksīdu, kurš izreaģē ar ūdeni un tad iegūst 99.9% etilspirtu.\nSintētiskais ceļš: sintētiski etilspirtu var iegūt no gāzes, kura tiek iegūta naftas pārstrādes procesā.\nRaksturlielumi: Qz(96%)=30000kJ/kg=6120kcal/kg; ts=78.3C; Sastāvs -> C-52%, H2-13%, O2-35%; blīvums=794kg/m3.\n Pielietojums: lieto kā piedevu benzīnam līdz 10% kā biopiedevu, kura arī paaugstina oktānskaitli.\n";
        static string s2_q13 = $"Dabasgāze, tās sastāvs, īpašības.";
        static string s2_a13 = $"Viens no nekaitīgākajiem fosīlā kurināmā veidiem, jo sadegot veidojas CO2 un ūdens tvaiks.Nav cieto daļiņu/pelnu.\nDabasgāzes sastāvs: CH4=93.5%, C2H6=2.6%, C3H8=0.9%, C4H10=0.3%, N2=2.7%. Siltumspēja Qz=8000-9080kcal/nm3=33500-33800kJ/nm3. Dabasgāze ir bez krāsas un smakas un cilvēks to nevar konstatēt, tāpēc gāzei pievieno odorantus (merkaptāns C2H5SH).\nVobes skaitlis dabasgāzei Iw=45-54MJ/nm3. H2S<0.2g/m3. Smakas koncentrācija gaisa balles ne mazāk kā 3.\nGaisa pārpalikums ir mazs un maza iespēja rasties slāpeklim. Vidējais izmešu daudzums: CO2=9.8%,CO=līdz 10 ppm, NOx=45mg/MJ, SO2=0mg/MJ, putekļi=0mg/MJ.\nAugstas automatizācijas iespējas, pārvadā caurulvados - nevajag uzglabāšanu, var lietot katlus kam nav nepieciešams operators, nav izdedžu. Trūkums: palielināta eksplozījas bīstamība un ugunsbīstamība, cenas pieaugums.\n";
        static string s2_q14 = $"Metāns, tā iegūšana no sadzīves atkritumiem.";
        static string s2_a14 = $"CH4. Atkritumu laukā veic urbumus, līdz pat metram diametrā, dziļumā līdz 25m un tajos vertikāli ievieto perfarētas caurules un telpu starp caurulēm un urbuma sienām piepilda ar, piemēram, šķembām. Augšējos galus caurulēm pievieno savākšanas kolektoriem pa kuriem gāzi sūknē uz attīrīšanas un savākšanas staciju. Lauku no virspuses noklāj ar blīvējošu kārtu (māls, etc), virs tā uzber kūdras vai zemes kārtu.\nJa neskaita ūdens tvaiku, tad sastāv no CH4-60%, CO2-38%, citas gāzes 2%. Ja ieskaita ūdens tvaiku, tad tas ir jāatdala. Gāzi izlaiž cauri saldēšanas iekārtai, mitrums izkrīt sniega veidā, bet biogāze iet tālāk. Iegūtās gāzes sildspēja Qz=3600-5000kcal/nm3. Ja gāzi papildus apstrādā tad palielinās siltumspēja - Var atdalīt sēra savienojumus, ja tādi ir. Samazināt CO2 daudzumu. Ja iegūst nepieciešamo vobes indexu, var iegūt arī augstāku siltumspēju.\nVar izmantot iekšdedzes motoros, tai skaitā koģenerācijas iekārtās, katlu iekārtās siltumu ieguvei, ja nodrošina kvalitātes standartus, tad var nodot kopējā dabasgāzes tīklā. Getliņos izmanto.\n";
        static string s2_q15 = $"Metāna ieguve no dzīvnieku mēsliem.";
        static string s2_a15 = $"Dzīvnieku mēslus atšķaida ūdenī iegūstot 8-12% šķīduma, var likt klāt arī zaļmasu. Šo izejmateriālu ievada reaktorā (tenkā), kur baktērijas sadala šo masu. Var būt periodiskas darbības vai nepārtrauktas darbības reaktori. Periodiskas parasti ir vertikāli - iekrauj, izstrādā, aizved. Un nepārtrauktas darbības, parasti horizontāli reaktori, var būt ar diametru vairāki metri un 6-10m garumā, un process ir nepārtraukts - pa vienu galu iekšā, pa otru ārā, nepārtraukti. Pārpalikumu var labi izmantot lauku mēslošanai, jo tas satur slāpekli, fosforu un kalcija savienojumus.\nTenkā process norisinās 2 stadijās. 1. stadijā baktērijas sadala masu vienkāršākās komponentēs - etiķskābē (CH3COOH), taukskābē (C2H5COOH) un nedaudz CO2 un amonjaks. Baktērijas nav īpaši prasīgas pret vides apstākļiem. 2. stadijā darbojas citas baktērijas, kuras ir ļoti jutīgas pret apstākļiem - tām ir nepieciešams konkrētas Ph līmenis (6.8-7.6) un svarīga ir temperatūra. Izšķir divu veidu baktērijas - 1. mezofīlās, kurām optimālā temperatūra ir ~35C, 2. termofīlās, kurām optimālā temperatūra ir ~55C. Šīs baktērijas pārstrādā skābes un iegūtā gāze, neskaitot mitrumu, sastāv no 60% metāna un 36-38% CO2. Mazos daudzumos var būt sērūdeņradis, amonjaks un citi.\nBaktērijas strādājot izdala siltumu ar kuru pietiek lai uzturētu 35C, bet nepietiek priekš 50 un vairāk grādiem. Tātad ir jāpievada papildus siltums un tur iztērē tos pašus 20% ko šeit iegūst vairāk. Mezofīlā procesā massa pilnībā izstrādājās līdz 18 dienām, termofīlā 5-6 dienas (3x ātrāk). Lielajās ražotnēs sasniegt šādu temperatūru ir ilgstoš process, tāpēc parasti lieto mezofīlo procesu uzturot 35C.\n";
        ...
        static string s2_a17 = $"Koksnes gazifikācijas rezultātā iegūst Singāzi. Process norisinās sekojoši: 1. stadijā notiek koksnes žūšana un temp 100-200C; tālāk koksne nonāk nākamajā zonā, kur pakāpeniski paaugstinot temperatūru līdz 500-700C bez skābekļa klātbūtnes, notiek koksnes termoķīmiskā sadalīšanās jeb pirolīze. Šajā procesā viedojas šķidrā, gāzveida un cietā frakcija.\ngāzveida satur CO, CO2, CH4, H20, H2, etc. Šķidrās: piroligniskās skābes. Cietā: koka ogle.\nlai koksne sadalītos ir nepieciešams noteikts daudzums ūdens tvaika, tāpēc reaktoram ir tvaika pievades sistēma 0.5daļas tvaika uz 1 masas daļu koksnes.\nIegūtās gāzes ķīmiskais sastāvs: CO=29%, H2=13%, CH4=4%, CO2=6,6%, N2=47,4%. Q=6550kJ/m3=1565kcal/m3. No kg šķeldas iegūst 1.7m3 gāzes. Šķidrā frakcija 0.081kg/kg.\n";
        static string s2_q18 = $"Insineratori, to uzbūve, veidi, darbība.";
        static string s2_a18 = $"Insineratori jeb atkritumu sadedzināšanas iekārtas. Šajās iekārtās sadedzina pa tiešo atkritumus nevis pārstrādā deggāzē. Pirms tam nepieciešama šķirošana. Lai uzturētu degšanas procesu normālā režīmā, izmanto papildus kurināmo, piemēram, dabasgāzi. Aparātam ir nepieciešama ļoti rūpīga dūmgāžu attīrīšana un pelni var saturēt kaitīgus savienojumus - dažādus metālus - šī iemesla dēļ ir speciāli noteiktumi pelnu uzglabāšanai. Atkarībā no tehnoloģijas ir 2 veidi: 1) bez atkritumvielu smalcināšanas - atkritumus atved un izkrauj šim nolūkam paredzētos laukumos vai telpās, tālāk tos šķiro atdalot nedegošos priekšmetus, parasti to dara manuāli uz konvejera, bet eksistē automātiskās šķirošanas iekārtas, kas atpazīst materiālus. Sašķirotos atkritumus nogādā speciālā telpā no kurienes ar iekrāvēju iekrauj katlu iekārtas priekškamerā. Tai vienmēr ir jābūt pilnai, lai neieplūstu gaiss. Tālāk ar hidrolisku iekārtu aktritumus iestumj uz ārdiem, kur norisinās degšana. Var būt slīpie ārdi un var būt kustīgie ārdi.\n2) Ar atkritumvielu sasmalcināšanu. Pēc izkraušanas un šķirošanas atkritumvielas sasmalcina gabaliņos ar izmēru ~2-2.5cm. Pēc tam ir magnētiskais filtrs, kas atdala metāliskās daļas. Tālāk šo degmasu pievada kurtuvei ar: 1. ar jaudīgiem ventilātoriem tajā iepūšot veidojas virpuļveida strūkla, kurā notiek degšanas process, pelni krīt uz leju, dūmgāzes plūst uz augšu; 2. izmanto pseidoverdošā slāņa kurtuves. Atkritumvielas ievada kurtuvē virs ārdiem caur kuriem ar lielu ātrumu pievada gaisa plūsmu. Gaiss paceļ kurināmā gabaliņus, kuri intensīvi deg no visām pusēm. Lai uzturētu temperatūru un atdalītu kurināmā daļiņas, reizēm kurtuvē atrodas smiltis.\n3. Atkritumu sadedzināšana ciklona kurtuvēs (priekškurtuvēs).\nKas jāievēro sadedzinot atkritumus: 1. obligāti degšanas zonā jāuztur augsta temperatūra (1100-1400C); 2. Atkritumiem parasti ir zema un mainīga sildspēja; 3. Atkritumi var saturēt kaitīgas vielas, kas nonāk pelnos un dūmgāzēs.\n";
        static string s2_q19 = $"Liesmas izplatīšanās ātrums (normālais, turbulentais). Laukumu likums.";
        static string s2_a19 = $"Liesmas izplatīšanās ātrums:\n - normālais: liesmas normālais izplatīšanās ātrums u{needsSubscript}n{needsSubscript} \n - turbulentais\n Laukuma likums:  \n";
        ...
        public static List<QuestionAndAnswers> GetS2QnAList()
        {
            var res = new List<QuestionAndAnswers>()
            {
                ...
                new QuestionAndAnswers(){ QuestionId = 7, Question = s2_q7, Answer = s2_a7, AnswerImage = s2_z7, Points = 1 },
                ...
                new QuestionAndAnswers(){ QuestionId = 40, Question = s2_q40, Answer = s2_a40, AnswerImage = zBlank, Points = 1 },
            };
